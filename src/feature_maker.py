...
class FeatureMaker():
    ...
    def transform(self, X):
        ...
        df["age_bmi"] = df["bmi"] * df["age"] / 100
        
        # BMI категории
        df["bmi_category"] = 0
        df.loc[df["bmi"] < 18.5, "bmi_category"] = 1
        df.loc[(df["bmi"] >= 18.5) & (df["bmi"] < 25), "bmi_category"] = 2
        df.loc[(df["bmi"] >= 25) & (df["bmi"] < 30), "bmi_category"] = 3
        df.loc[(df["bmi"] >= 30) & (df["bmi"] < 35), "bmi_category"] = 4
        df.loc[df["bmi"] >= 35, "bmi_category"] = 5
